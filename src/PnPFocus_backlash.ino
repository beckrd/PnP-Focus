...
//
// Backlash compensation added by RD Beck (beckrd@gmail.com). 18 August 2018
...
long Backlash = 24;  //change this line for different backlash
float tempC = 0;
long timer_millis = 0;
boolean BacklashComp = false;  //flag for backlash compensation
...
void loop(){
   ...
   switch (lcd_key){             
    ...
    if (isRunning) {
      if (distanceToGo > 0) {
        ...
        else if (distanceToGo == 0 && BacklashComp)  //backlash compensation correction
         {
           distanceToGo = -1.0 * Backlash;
           BacklashComp = false;
         }
  ...
  if (eoc) {
    ...
    if (!strcasecmp(cmd, "SN")) {
      ...
      if (distanceToGo > 0)  //backlash compensation applied
      {
        distanceToGo += Backlash;
        BacklashComp = true;
      }
    }
